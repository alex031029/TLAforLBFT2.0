...
CONSTANTS validators, f \* a set of validator, and f as the number of faulty nodes
CONSTANTS quorum   \* a set of quorums, each element of which is a set of validators 
CONSTANTS highestHeight, possibleHeights    \*legal heights
CONSTANTS validatorTimesHeight  \* validators \X possibleHeights
CONSTANTS proposers \* a set of proposers
CONSTANTS predeterminedBlockHeight  \* each proposer is assigend a height to proposer a block
CONSTANTS sig   \* signature for each validators


\* assumptions
ASSUME Cardinality(validators) = 3*f+1
ASSUME \A q \in quorum : q \in SUBSET(validators) 
ASSUME Cardinality(possibleHeights) = highestHeight
ASSUME Cardinality(proposers) = highestHeight
ASSUME validatorTimesHeight = validators \X possibleHeights 
...
define
    ...
    prepareCertificate(v) ==
        \E q \in quorum : q \subseteq prepareSig[v]

    commitCertificate(v) ==
        \E q \in quorum : q \subseteq commitSig[v]       
...
\*    allValidated == 
\*        <>(\A va \in validators: validatorBlockHeight[va] = highestHeight + 1)
...
fair+ process validator \in validators
...
Fsm:
    while ~consensus do
        either \* idle state
            ...
            await blockReceiver[<<self, 1>>] \in DOMAIN predeterminedBlockHeight; 
            \* requirement of the block height of the validator
            await predeterminedBlockHeight[blockReceiver[<<self, 1>>]] = validatorBlockHeight[self]; 
            blockCache[self] := blockReceiver[<<self, 1>>];
...
SendBlock1:    
    blockReceiver[<<"v1", 1>>] := self;
SendBlock2:     
    blockReceiver[<<"v2", 1>>] := self;
SendBlock3:      
    blockReceiver[<<"v3", 1>>] := self;
SendBlock4: 
    blockReceiver[<<"v4", 1>>] := self;     
...
\* BEGIN TRANSLATION
\* Label broadcastAll of procedure broadcastAll at line 130 col 5 changed to broadcastAll_
\* Parameter inputType of procedure addSig at line 88 col 28 changed to inputType_
\* Parameter sender of procedure addSig at line 88 col 39 changed to sender_
\* Parameter sender of procedure broadcast at line 115 col 21 changed to sender_b
\* Parameter inputType of procedure broadcast at line 115 col 29 changed to inputType_b
CONSTANT defaultInitValue
VARIABLES state, prepareSig, commitSig, blockCache, blockReceiver, 
...
vars == << state, prepareSig, commitSig, blockCache, blockReceiver, 
           validatorBlockHeight, proposerBlockHeight, validatorChain, 
...
PrepareMsg(self) == /\ pc[self] = "PrepareMsg"
                    ...
                    /\ UNCHANGED << state, commitSig, blockCache, 
...
CommitMsg(self) == /\ pc[self] = "CommitMsg"
                   ...
                   /\ UNCHANGED << state, blockCache, blockReceiver, 
...
Broadcast1(self) == /\ pc[self] = "Broadcast1"
                    ...
                    /\ UNCHANGED << state, prepareSig, commitSig, blockCache, 
                                    blockReceiver, validatorBlockHeight, 
                                    proposerBlockHeight, validatorChain, 
...
Fsm(self) == /\ pc[self] = "Fsm"
             /\ IF ~consensus[self]
                   THEN /\ \/ /\ state[self] = 0
                              /\ blockReceiver[<<self, 1>>] \in DOMAIN predeterminedBlockHeight
                              /\ predeterminedBlockHeight[blockReceiver[<<self, 1>>]] = validatorBlockHeight[self]
                              /\ blockCache' = [blockCache EXCEPT ![self] = blockReceiver[<<self, 1>>]]
                   ...
                   ELSE /\ pc' = [pc EXCEPT ![self] = "Done"]
                        /\ UNCHANGED << state, prepareSig, commitSig, 
                                        ...
                                        inputType_b >>
             /\ UNCHANGED << blockReceiver, validatorBlockHeight, 
                             proposerBlockHeight, validatorChain, 
...
SendBlock1(self) == /\ pc[self] = "SendBlock1"
                    /\ blockReceiver' = [blockReceiver EXCEPT ![<<"v1", 1>>] = self]
...
SendBlock2(self) == /\ pc[self] = "SendBlock2"
                    /\ blockReceiver' = [blockReceiver EXCEPT ![<<"v2", 1>>] = self]
...
SendBlock3(self) == /\ pc[self] = "SendBlock3"
                    /\ blockReceiver' = [blockReceiver EXCEPT ![<<"v3", 1>>] = self]
...
SendBlock4(self) == /\ pc[self] = "SendBlock4"
                    /\ blockReceiver' = [blockReceiver EXCEPT ![<<"v4", 1>>] = self]
...
Spec == /\ Init /\ [][Next]_vars
        /\ \A self \in validators : /\ SF_vars(validator(self))
                                    /\ SF_vars(broadcast(self))
                                    /\ SF_vars(broadcastAll(self))
                                    /\ SF_vars(addSig(self))
...
\* Last modified Tue Sep 17 20:28:22 CST 2019 by Dell
\* Created Wed Aug 07 14:38:34 CST 2019 by Dell
